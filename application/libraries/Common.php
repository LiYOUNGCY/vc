...
class Common
{
    ...
    static function extract_topic($article_id, $article_title, $article_content)
    {
        $content = array(
            'article_id' => $article_id,
            'article_title' => $article_title,
            'article_content' => Common::extract_content($article_content),
            'article_image' => Common::extract_first_img_by_topic($article_content)
        );
        return $content;
    }
    ...
    static function extract_first_img_by_topic($article_content)
    {
        $match = array();
        preg_match_all("/<[img|IMG].*?src=[\'|\"](.*?(?:[\.gif|\.jpg]))[\'|\"].*?[\/]?>/", $article_content, $match);
        if ($match[1][0]) {
            return Common::get_thumb_url_by_suffix($match[1][0]);
        }
        return null;
    }
...
//            $filename = 'f011597f8615be412c8fad36e7c0f694_thumb.jpg';
    ...
    static function get_thumb_url_by_suffix($path)
    {
        $image = explode('/', $path);
        if (!empty($image)) {
            $len = count($image);
